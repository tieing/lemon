...
func TestQueueLength(t *testing.T) {
	...
	for i := 0; i < 1000000; i++ {
		q.Remove()
		if q.Length() != 1000000-i-1 {
			t.Error("removing: queue with", 1000000-i-i, "elements has length", q.Length())
		}
	}
}
...
func TestQueueTickRemove(b *testing.T) {
	q := New()
	g := sync.WaitGroup{}
	...
	g.Wait()
	t2 := time.Now().Sub(t).Nanoseconds()
	println(t2 / int64(g1*g2)) // 113  Nanoseconds
	println(q.Length())        // 100000000
}
